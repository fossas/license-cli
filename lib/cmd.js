...
function getLocator (options) {
  return Promise.resolve().then(function getRevision () {
    var project_url = url.resolve(options.endpoint, '/api/revisions/' + encodeURIComponent(options.project) + '/build')
    if (!options.revision) {
    ...
    return options.project.replace('https://', '').replace('http://', '') + '$' + options.revision
  })
}
...
function queueBuild (locator, options, indicator) {
  ...
  .then(function handleBuild (build) {
    return Promise.resolve().then(function () {
      ...
      var buildStatus = build.task ? build.task.status : 'CREATED'
      switch (buildStatus) {
        case 'FAILED':
...
function renderScanResults (issues, indicator) {
  if (issues.length === 0) return indicator.succeed('No license issues found from your FOSSA scan'.green)
